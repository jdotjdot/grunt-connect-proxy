...
utils.matchContext = function(context, url) {
    var positiveContexts, negativeContexts, positiveMatch, negativeMatch, regexContexts;
    var contexts = context;
    if (!_.isArray(contexts)) {
        contexts = [contexts];
    }

    regexContexts = _.filter(contexts, function (c){return c instanceof RegExp;});
    contexts = _.filter(contexts, function(c){return !(c instanceof RegExp);});

    // if context matches a regex, return true, otherwise continue
    for (var i = 0; i < regexContexts.length; i++) {
        if (regexContexts[i].test(url)) {
            return true
        }
    }
